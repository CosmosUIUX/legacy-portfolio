...
describe('useMotion hook', () => {
  ...
  it('should apply performance adjustments', () => {
    ...
    expect(result.current.animationProps.transition.duration).toBeLessThan(DURATION_PRESETS.normal / 1000)
  })
})
...
describe('usePerformantAnimation hook', () => {
  it('should optimize animation config based on performance mode', () => {
    ...
    // Duration should be reduced in battery mode
    expect(result.current.optimizedConfig.duration).toBeLessThan(baseConfig.duration)
  })
...
describe('useTextAnimation hook', () => {
  ...
  it('should handle different animation types', () => {
    ...
    // Test fade-in-words
    const { result: fadeResult } = renderHook(() => useTextAnimation({
      text,
    ...
    // Test slide-in-chars
    const { result: slideResult } = renderHook(() => useTextAnimation({
      text,
...
describe('useGestureAnimation hook', () => {
  ...
  it('should support touch events', () => {
    ...
    act(() => {
      result.current.gestureProps.onTouchStart({ touches: [{ clientX: 100, clientY: 0 }] })
    })

    act(() => {
      result.current.gestureProps.onTouchMove({ touches: [{ clientX: 40, clientY: 0 }] })
    })
