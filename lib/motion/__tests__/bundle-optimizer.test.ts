...
describe("Bundle Optimizer", () => {
    ...
    it("should load different component types", async () => {
      ...
      for (const componentName of components) {
        ...
        expect(component).toBe(
          `Mocked${componentName.charAt(0).toUpperCase() + componentName.slice(1)}`,
        );
      }
    });

    it("should handle unknown components", async () => {
      await expect(loadMotionComponent("unknownComponent")).rejects.toThrow(
        "Unknown Motion.dev component",
      );
    });
  ...
  describe("preloadCriticalComponents", () => {
    it("should preload critical components", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        "Critical Motion.dev components preloaded successfully",
      );
    });
    ...
    it("should handle preload failures gracefully", async () => {
      ...
      jest
        .spyOn(require("../bundle-optimizer"), "loadMotionComponent")
  ...
  describe("clearComponentCache", () => {
    it("should clear the component cache", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        "Motion.dev component cache cleared",
      );
    });
  });
});
...
describe("analyzeBundleUsage", () => {
  ...
  it("should analyze bundle usage in development mode", async () => {
    ...
    expect(consoleSpy.log).toHaveBeenCalledWith("Loaded Components:", [
      "motion",
    ]);
