  ...
  describe("CPU Performance Tests", () => {
    it("should optimize performance under CPU load", async () => {
      const cpuLoadTest = async () => {
        ...
        const firstHalfAvg =
          firstHalfFrames.reduce((sum, time) => sum + time, 0) /
          firstHalfFrames.length;
        const secondHalfAvg =
          secondHalfFrames.reduce((sum, time) => sum + time, 0) /
          secondHalfFrames.length;
    ...
    it("should adapt to different performance modes", async () => {
      const performanceModeTest = async (
      ...
      const staggerPerformanceTest = async (itemCount: number) => {
        ...
        const staggerAnimation = () => {
          ...
          for (let i = 0; i < itemCount; i++) {
            ...
            const easedProgress =
              progress < 1 ? Math.sin((progress * Math.PI) / 2) : 1;
  ...
  describe("Real-world Performance Scenarios", () => {
    ...
    it("should handle scroll-triggered animations efficiently", async () => {
      const scrollAnimationTest = async () => {
        ...
        const scrollAnimation = () => {
          ...
          scrollElements.forEach((element, index) => {
            ...
            if (element.inView) {
              ...
              element.progress = Math.max(
                0,
                Math.min(1, (400 - Math.abs(distance)) / 400),
              );
        ...
        const averageFrameTime =
          frameTimes.reduce((sum, time) => sum + time, 0) / frameTimes.length;
        const elementsAnimated = scrollElements.filter(
          (el) => el.progress > 0,
        ).length;
