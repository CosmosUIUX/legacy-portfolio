...
function TestErrorRecoveryComponent() {
  const { hasErrors, errorCount, reportError, clearErrors } =
    useErrorRecovery();
  ...
  return (
    <div>
      <div data-testid="error-status">Has errors: {hasErrors.toString()}</div>
      <div data-testid="error-count">Error count: {errorCount}</div>
...
describe("AnimationErrorBoundary", () => {
  ...
  it("should render custom fallback component", () => {
    ...
    expect(
      screen.getByText("Custom fallback: Test animation error"),
    ).toBeInTheDocument();
  });
...
describe("AnimationErrorLogger", () => {
  ...
  it("should filter errors by type", () => {
    ...
    const runtimeErrors = (AnimationErrorLogger as any).getErrorsByType(
      AnimationErrorType.ANIMATION_RUNTIME_ERROR,
    );
