...
describe('Accessibility Compliance Tests', () => {
  ...
  describe('WCAG 2.1 Compliance', () => {
    describe('Guideline 2.2.2 - Pause, Stop, Hide', () => {
      it('should respect prefers-reduced-motion setting', () => {
        ...
        const reducedMotionTest = AccessibilityTestUtils.testReducedMotion(animatedElement)
        expect(reducedMotionTest.hasReducedMotionStyles || 
               reducedMotionTest.animationDuration === '0.01ms' ||
               reducedMotionTest.transitionDuration === '0.01ms').toBe(true)
      })

      it('should disable animations when reduced motion is preferred', async () => {
        renderWithMotion(
          <AccessibleAnimationComponent />,
          { providerProps: { reducedMotion: true } }
        )
    ...
    describe('Guideline 2.3.1 - Three Flashes or Below Threshold', () => {
      ...
      it('should detect unsafe flash rates', () => {
        ...
        expect(unsafeAnimation.warnings).toContain('Flash rate exceeds safe threshold (3 flashes/second)')
      })
      ...
      it('should detect excessive color changes', () => {
        ...
        expect(unsafeAnimation.safe).toBe(false)
        expect(unsafeAnimation.warnings).toContain('Too many rapid color changes detected')
      })
      ...
      it('should detect insufficient contrast', () => {
        ...
        expect(unsafeAnimation.safe).toBe(false)
        expect(unsafeAnimation.warnings).toContain('Insufficient color contrast ratio (minimum 4.5:1)')
      })
    })
    ...
    describe('Guideline 4.1.2 - Name, Role, Value', () => {
      it('should provide proper ARIA attributes', () => {
        ...
        expect(button.getAttribute('aria-label')).toBe('Accessible animation button')
      })
  ...
  describe('Screen Reader Compatibility', () => {
    ...
    it('should not interfere with screen reader navigation', () => {
      renderWithMotion(<AccessibleAnimationComponent animationState="animating" />)
