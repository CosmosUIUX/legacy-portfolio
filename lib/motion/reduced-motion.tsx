...
export function ReducedMotionProvider({
  ...
  const [respectSystemPreference, setRespectSystemPreference] =
    useState(defaultRespectSystem);
  const [userMotionPreference, setUserMotionPreference] = useState<
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "respect-system-motion",
      respectSystemPreference.toString(),
    );
      ...
      <fieldset>
        ...
        <div className="current-status">
          <p>
            Current status:{" "}
            <strong>
              {reducedMotion ? "Reduced motion" : "Animations enabled"}
            </strong>
          </p>
        </div>
      </fieldset>
...
export function useMotionCapability() {
  ...
  useEffect(() => {
    ...
    const supportsAnimations =
      "animate" in testElement || "webkitAnimate" in testElement;
    ...
    const supportsGPUAcceleration =
      "transform3d" in testElement.style ||
