...
export function useFormValidation(options: UseFormValidationOptions = {}) {
  ...
  const { onValidationChange, animateOnError = true, animateOnSuccess = true } = options

  const validate = useCallback(async (result: ValidationResult) => {
...
export const validators = {
  ...
  minLength: (minLength: number) => (value: string): ValidationResult => {
    ...
    return {
      isValid,
      message: !isValid ? `Must be at least ${minLength} characters` : undefined,
      state: !isValid ? 'error' : 'success'
    }
  },
  ...
  maxLength: (maxLength: number) => (value: string): ValidationResult => {
    ...
    return {
      isValid,
      message: !isValid ? `Must be no more than ${maxLength} characters` : undefined,
      state: !isValid ? 'error' : 'success'
    }
  },
 ...
 */
export function composeValidators(...validators: Array<(value: string) => ValidationResult>) {
