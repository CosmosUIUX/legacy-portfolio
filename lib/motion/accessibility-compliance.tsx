...
export function AccessibilityCompliance({
    ...
    if (containerRef.current) {
      ...
      if (skipLinks.length === 0 && interactiveElements.length > 5) {
        recommendations.push(
          "Consider adding skip navigation links for better keyboard accessibility",
        );
      }
    }
  ...
  useEffect(() => {
    if (complianceReport && enableReporting) {
      const errorCount = complianceReport.issues.filter(
        (issue) => issue.severity === "error",
      ).length;
      const warningCount = complianceReport.issues.filter(
      ...
      if (errorCount === 0 && warningCount === 0) {
        ...
        announce(
          `Accessibility issues found: ${errorCount} errors, ${warningCount} warnings`,
          "assertive",
        );
      }
    }
      ...
      {process.env.NODE_ENV === "development" && complianceReport && (
        <div
          ...
          style={{
            ...
            color: "white",
            backgroundColor:
              complianceReport.issues.filter((i) => i.severity === "error")
                .length === 0
                ? "#22c55e"
                : "#ef4444",
            zIndex: 10000,
