  ...
  useEffect(() => {
    const cleanup = setInterval(() => {
      ...
      setAnnouncements((prev) => {
        ...
        Array.from(prev.entries()).forEach(([id, announcement]) => {
          if (now - announcement.timestamp < 30000) {
            // Keep for 30 seconds
...
export function AnimationAnnouncer({
  ...
  useEffect(() => {
    if (previousStateRef.current !== animationState) {
      const message =
        effectiveMessages[animationState as keyof typeof effectiveMessages];
      if (message) {
        const effectivePriority =
          animationState === "error" ? "assertive" : priority;
  ...
  useEffect(() => {
    if (errors) {
      ...
      if (newErrors.length > 0) {
        const errorMessages = newErrors.map(
          ([field, error]) => `${field}: ${error}`,
        );
        announce(
          `Form validation errors: ${errorMessages.join(", ")}`,
          "assertive",
        );
      }
...
export function useContextualAnnouncements() {
  ...
  const announceProgress = (current: number, total: number, label?: string) => {
