...
export function useKeyboardNavigation() {
    ...
    const handleFocusOut = (event: FocusEvent) => {
      ...
      setTimeout(() => {
        if (
          ...
          const lastFocusable = focusHistory.find(
            (el) =>
  ...
  const preserveFocus = useCallback(() => {
    if (
      ...
      document.contains(focusedElementRef.current)
    ) {
  ...
  const triggerAlternativeAction = useCallback(
    (
      ...
      if (
        audioFeedback &&
        isAudioEnabled &&
        (interactionMode === "audio" || interactionMode === "visual")
      ) {
        try {
          const AudioContext =
            window.AudioContext || (window as any).webkitAudioContext;
      ...
      if (
        hapticFeedback &&
        isHapticEnabled &&
        (interactionMode === "haptic" || interactionMode === "visual")
      ) {
        try {
          ...
          switch (action) {
            ...
            case "click":
              pattern =
                intensity === "low" ? 30 : intensity === "high" ? 100 : 50;
              break;
            case "hover":
            case "focus":
              pattern = [20];
      ...
      if (visualFeedback && element) {
        ...
        const resetDuration =
          intensity === "low" ? 150 : intensity === "high" ? 300 : 200;
        setTimeout(() => {
  ...
  switch (element) {
    case "button":
      ...
      baseProps["aria-pressed"] =
        state === "animating" || state === "complete" ? "true" : "false";
      break;
    case "link":
      baseProps["role"] = "link";
      break;
...
export function generateAccessibilityInstructions(
  ...
  const componentSpecificInstructions: Record<
    string,
    {
      keyboard: string[];
      screenReader: string[];
  ...
  const componentInstructions = componentSpecificInstructions[
    componentType
  ] || {
    keyboard: [],
    screenReader: [],
  ...
  const generalInstructions =
    [
      ...
      customInstructions ||
        "Use your preferred input method to interact with this component",
    ].join(". ") + ".";
