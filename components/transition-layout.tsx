...
import { RouteTransition, useFocusManagement, usePageTransition, TransitionProgress } from "./page-transition"
...
export function TransitionLayout({ 
  ...
  const getTransitionDirection = React.useCallback((newPath: string, oldPath: string) => {
  ...
  const [currentDirection, setCurrentDirection] = React.useState<'forward' | 'backward' | 'none'>('none')
...
export function TransitionLink({ 
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (shouldAnimate) {
      ...
      navigateWithTransition(href, direction === 'auto' ? 'forward' : direction, {
        replace,
        scroll
      })
    } else {
