...
describe("ProductGrid", () => {
  ...
  it("renders products in grid layout", () => {
    render(
      <ProductGrid products={mockProducts} onQuickLook={mockOnQuickLook} />,
    );
  ...
  it("filters products correctly with smooth transitions", async () => {
    ...
    await waitFor(
      () => {
        expect(screen.getByText("Modern Kitchen")).toBeInTheDocument();
        expect(
          screen.queryByText("Elegant Dining Set"),
        ).not.toBeInTheDocument();
        expect(
          screen.queryByText("Contemporary Staircase"),
        ).not.toBeInTheDocument();
      },
      { timeout: 1000 },
    );
  ...
  it("shows enhanced empty state when no products match filter", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('No products match the "limited" filter'),
      ).toBeInTheDocument();
