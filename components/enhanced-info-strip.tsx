            ...
            return (
              <motion.div
                ...
                transition={{
                  ...
                  duration: DURATION_PRESETS.normal / 1000,
                  ease: (animationType === "bounce"
                    ? EASING_PRESETS.bounce
                    : EASING_PRESETS.cinematic) as any,
                }}
                className="flex items-center gap-2 group cursor-pointer"
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                whileHover={
                  enableHoverEffects && performanceMode !== "battery"
                    ? {
                        scale: 1.08,
                        y: -3,
                        transition: {
              ...
              >
                ...
                <motion.div
                  ...
                  transition={{
                    ...
                    ease:
                      item.iconAnimation === "bounce"
                        ? (EASING_PRESETS.bounce as any)
                        : (EASING_PRESETS.cinematic as any),
                  }}
                  whileHover={
                    enableHoverEffects && performanceMode !== "battery"
                      ? {
                          rotate:
                            hoveredIndex === index ? [0, -8, 8, -4, 4, 0] : 0,
...
export function ResponsiveInfoStrip({
  ...
  const { animationType, staggerDelay, enableAdvancedEffects } =
  ...
  const enhancedItems = items.map((item, index) => ({
    ...
    iconAnimation: enableAdvancedEffects
      ? item.iconAnimation ||
        (["rotate", "flip", "bounce", "scale"][index % 4] as any)
      : "fade",
  }));
