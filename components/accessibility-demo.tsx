...
import { useReducedMotion, useKeyboardNavigation, useAlternativeInteractions } from '@/lib/motion/accessibility'
...
function AccessibilityDemoContent() {
  ...
  const { interactionMode, availableInteractionMethods } = useAlternativeInteractions()
    ...
    <AccessibilityCompliance enableCompliance={true} enableReporting={true}>
      ...
      <div className="p-8 max-w-4xl mx-auto">
        <SkipTarget id="main-content" label="Main content area" as="main">
          <h1 className="text-3xl font-bold mb-8">Accessibility Features Demo</h1>
          ...
          <section className="mb-8">
            ...
            {showSettings && (
              <MotionPreferenceSettings className="mb-4" />
            )}
            ...
            <div className="p-4 bg-gray-100 rounded-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <strong>System Reduced Motion:</strong> {reducedMotion ? 'Enabled' : 'Disabled'}
                </div>
                <div>
                  <strong>Context Reduced Motion:</strong> {contextReducedMotion ? 'Enabled' : 'Disabled'}
                </div>
                <div>
                  <strong>Keyboard User:</strong> {isKeyboardUser ? 'Yes' : 'No'}
                </div>
                <div>
          ...
          <section className="mb-8">
            <h2 className="text-2xl font-semibold mb-4">Interactive Button with Accessibility</h2>
            <FocusBoundary 
              label="Animation demo section"
              onEnter={() => announceInteraction('focus', 'animation demo section')}
            >
              <MotionPreference 
                fallback={
                  <button
                ...
                >
                  <motion.button
                    ...
                    animate={{
                      ...
                      backgroundColor: animationState === 'complete' ? '#16a34a' : '#2563eb'
                    }}
                    transition={{
                  ...
                  >
                    <MotionPreference
                      fallback={
                        <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4">
                          <h3 className="text-xl font-semibold mb-4">Accessible Modal (Static)</h3>
                          <p className="mb-4">
                            This modal demonstrates proper focus management, keyboard navigation, 
                            and screen reader support. Animations are disabled due to motion preferences.
                          </p>
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: contextReducedMotion ? 0 : 0.2 }}
                      >
                        <h3 className="text-xl font-semibold mb-4">Accessible Modal</h3>
                        <p className="mb-4">
                          This modal demonstrates proper focus management, keyboard navigation, 
                          and screen reader support. Try using Tab to navigate and Escape to close.
                        </p>
                        <div className="flex gap-2">
                          <button
                            onClick={handleModalClose}
          ...
          <SkipTarget id="card-grid" label="Card grid section">
            <section className="mb-8">
              <h2 className="text-2xl font-semibold mb-4">Accessible Card Grid</h2>
                    ...
                    <MotionPreference
                      fallback={
                        <div
                          ...
                          tabIndex={0}
                          onClick={() => announceInteraction('click', `card ${item}`)}
                        >
                          <h3 className="font-semibold mb-2">Card {item} (Static)</h3>
                          <p className="text-gray-600">
                            This card demonstrates accessible interactions without animations.
                          </p>
                        </div>
                      }
                    >
                      ...
                      >
                        ...
                        <p className="text-gray-600">
                          This card demonstrates accessible hover effects and keyboard navigation.
                        </p>
                      </motion.div>
                    </MotionPreference>
                  </AccessibilityWrapper>
          ...
          <SkipTarget id="instructions" label="Accessibility instructions">
            <section className="mb-8 p-4 bg-blue-50 rounded-lg">
              <h2 className="text-xl font-semibold mb-4">Accessibility Instructions</h2>
              <div className="space-y-2 text-sm">
                <p><strong>Skip Navigation:</strong> Press Tab at the top of the page to reveal skip links</p>
                <p><strong>Keyboard Navigation:</strong> Use Tab to navigate, Enter/Space to activate, Escape to close modals</p>
                <p><strong>Screen Readers:</strong> All interactions are announced with proper ARIA labels and live regions</p>
                <p><strong>Reduced Motion:</strong> Animations are disabled or reduced when motion preferences are set</p>
                <p><strong>Alternative Feedback:</strong> Audio and haptic feedback available where supported</p>
                <p><strong>Focus Management:</strong> Focus is properly trapped in modals and preserved during animations</p>
                <p><strong>Motion Settings:</strong> Use the settings panel above to customize motion preferences</p>
              </div>
            </section>
          </SkipTarget>
          ...
          <section className="p-4 bg-yellow-50 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              <p>• Try navigating with keyboard only (Tab, Enter, Space, Escape)</p>
              <p>• Enable "Reduce motion" in your system preferences to see animations disable</p>
              <p>• Use the motion preference settings above to test different configurations</p>
              <p>• Use a screen reader to test announcements and navigation</p>
              <p>• Check the browser console for accessibility compliance reports</p>
              <p>• Test with high contrast mode and forced colors</p>
              <p>• Try the skip navigation links by pressing Tab at the top of the page</p>
            </div>
          </section>
        </SkipTarget>
      </div>
