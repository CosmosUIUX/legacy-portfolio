  ...
  const getAnimationVariants = () => {
    ...
    const hoverVariants = {
      ...
      hover: {
        ...
        boxShadow: animationPreset === 'subtle'
        ...
        {product.badge && (
          <motion.div
            ...
            whileHover={{
              scale: 1.1,
              rotate: [0, -2, 2, 0],
              transition: { duration: 0.3, rotate: { duration: 0.6, repeat: Infinity, ease: "easeInOut" } }
            }}
            whileTap={{ scale: 0.95, rotate: 0, transition: { duration: 0.1 } }}
          >
            <motion.span
              className={cn(
                "px-3 py-1 text-xs font-medium rounded-full backdrop-blur-sm relative overflow-hidden cursor-pointer",
                ...
                product.badge === "Featured" && "bg-indigo-500/90 text-white",
              )}
              animate={{
                boxShadow: [
        ...
        <div className="relative overflow-hidden" style={{ aspectRatio: "25/36" }}>
          <div className="relative w-full h-full">
            ...
            <motion.div
              ...
              animate={{ backgroundColor: hoverMotion.isHovered ? "rgba(0, 0, 0, 0.1)" : "rgba(0, 0, 0, 0)" }}
            ...
            >
              ...
              <motion.span
                ...
                animate={{
                  ...
                  y: 0,
                  color: priceKey > 0 ? ["#ffffff", "#10b981", "#ffffff"] : "#ffffff"
                }}
                transition={{ delay: 0.6, duration: priceKey > 0 ? 0.8 : 0.4, ease: "easeOut" }}
              >
                <motion.span
                  className="relative z-10"
                  animate={priceKey > 0 ? {
                    filter: ["brightness(1)", "brightness(1.3) saturate(1.2)", "brightness(1)"]
                  } : {}}
                  transition={{ duration: 0.6, ease: "easeOut" }}
                >
                  {product.price.split('').map((char, index) => (
                    <motion.span
                      ...
                      transition={{ delay: priceKey > 0 ? index * 0.05 : 0, duration: 0.3, ease: "easeOut" }}
                    >
                      {char}
                    </motion.span>
                  ))}
