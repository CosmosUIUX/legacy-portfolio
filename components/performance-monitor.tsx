...
import { globalPerformanceDashboard, globalMemoryManager } from '../lib/motion/performance'
...
export function PerformanceMonitor({ 
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const animationSummary = globalPerformanceDashboard.getPerformanceSummary()
      const activeAnimations = globalMemoryManager.getActiveCount()
      const recentAlerts = globalPerformanceDashboard.getAlerts(Date.now() - 60000) // Last minute

      setMetrics({
        loadTime: navigationEntry ? Math.round(navigationEntry.loadEventEnd - navigationEntry.fetchStart) : 0,
        domContentLoaded: navigationEntry ? Math.round(navigationEntry.domContentLoadedEventEnd - navigationEntry.fetchStart) : 0,
        ...
        memoryUsed: memoryInfo ? Math.round(memoryInfo.usedJSHeapSize / 1024 / 1024) : 0,
        memoryTotal: memoryInfo ? Math.round(memoryInfo.totalJSHeapSize / 1024 / 1024) : 0,
        timestamp: Date.now()
      })
  ...
  return (
    <div className={`fixed ${positionClasses[position]} z-50 bg-black/90 text-white text-xs p-3 rounded-lg font-mono shadow-lg border border-gray-700`}>
      <div className="flex items-center gap-2 mb-2">
        <div className={`w-2 h-2 rounded-full animate-pulse ${
          animationMetrics?.summary?.overallHealth === 'good' ? 'bg-green-400' :
          animationMetrics?.summary?.overallHealth === 'fair' ? 'bg-yellow-400' :
          animationMetrics?.summary?.overallHealth === 'poor' ? 'bg-red-400' : 'bg-gray-400'
        }`}></div>
        ...
        <div className="space-y-2 min-w-[250px]">
          ...
          <div className="space-y-1">
            ...
            {metrics.memoryUsed > 0 && (
              <div>Memory: {metrics.memoryUsed}MB / {metrics.memoryTotal}MB</div>
            )}
          ...
          {animationMetrics?.summary && (
            <div className="border-t border-gray-600 pt-2 space-y-1">
              <div className="text-gray-300 font-semibold flex items-center gap-2">
                Animation Performance
                <span className={getHealthColor(animationMetrics.summary.overallHealth)}>
                  {getHealthIndicator(animationMetrics.summary.overallHealth)}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>FPS: {animationMetrics.summary.averageFrameRate}</div>
                <div>Components: {animationMetrics.summary.totalComponents}</div>
                <div>Active: {animationMetrics.activeAnimations}</div>
                <div>Memory: {animationMetrics.summary.averageMemoryUsage}MB</div>
              </div>
              
              {(animationMetrics.errorCount > 0 || animationMetrics.warningCount > 0) && (
                <div className="flex gap-2 text-xs">
                  {animationMetrics.errorCount > 0 && (
                    <span className="text-red-400">⚠ {animationMetrics.errorCount} errors</span>
                  )}
                  {animationMetrics.warningCount > 0 && (
                    <span className="text-yellow-400">⚠ {animationMetrics.warningCount} warnings</span>
                  )}
                </div>
              )}
            </div>
          ...
          {showDetails && (
            <div className="border-t border-gray-600 pt-2 space-y-1">
              ...
              <div className="text-xs">
                ...
                {animationMetrics?.summary && (
                  <div>Render Time: {animationMetrics.summary.averageRenderTime}ms</div>
                )}
              </div>
